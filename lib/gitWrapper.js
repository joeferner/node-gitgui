...
exports.log = function (gitPath, callback) {
  async.auto({
    ...
    if (results.status.hasWorkingCopy) {
      ...
      var refMatches = results.refs
        .filter(function (r) { return r.name === 'refs/heads/' + results.status.currentBranch; });
      if (refMatches && refMatches.length === 1) {
        workingCopy.parents = [ refMatches[0].id ];
      }
...
var getStatus = exports.getStatus = function (gitPath, callback) {
  runGit(gitPath, ['remote', 'update'], function (err) {
    ...
    hasWorkingCopy(function (err, workingCopy) {
      if (err) {
        return callback(err);
      }
