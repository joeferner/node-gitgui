...
exports.stashDrop = function(gitPath, stashId, callback) {
  runGit(gitPath, ['stash', 'drop', 'stash@{' + stashId + '}'], function(err, results) {
    if (err) {
      return callback(err);
    }
    return callback();
...
var getStatus = exports.getStatus = function(gitPath, callback) {
  ...
  function getStatus(callback) {
    return runGit(gitPath, ['status'], function(err, results) {
      ...
      m = results.match(/Your branch is behind '.*' by ([0-9]*) commit/);
      if (m) {
...
function parseStatus(results) {
  var lastLineRename = null;
  return results
    .split('\0')
    .filter(function(line) { return line; })
    .map(function(line) {
      if (lastLineRename) {
        lastLineRename.renamedTo = line;
        return null;
      }
      var parts = line.match(/^(..) (.*)$/);
      if (!parts) {
        throw new Error("Unexpected status results: " + line);
      }
...
exports.checkout = function(gitPath, branch, newBranchName, callback) {
  var args = ['checkout'];
...
exports.checkoutFile = function(gitPath, filename, callback) {
  var args = ['checkout'];
  args.push(filename);
  runGit(gitPath, args, function(err, results) {
    if (err) {
...
function runGit(gitPath, args, options, callback) {
  ...
  var child;
  if (gitExeFilename.toLowerCase().indexOf('.cmd') >= 0) {
    child = spawn('cmd', ['/c', gitExeFilename].concat(args), opts);
  } else {
    child = spawn(gitExeFilename, args, opts);
  }
