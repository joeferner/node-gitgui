...
function postLocalFilename(req, res, next) {
  ...
  case 'checkout':
    return checkoutLocalFile(repoName, filename, function(err) {
      if (err) {
        return next(err);
      }
      return res.end('ok');
  ...
  function checkoutLocalFile(repoName, filename, callback) {
    filename = path.join(repoName, filename);
    return git.checkoutFile(repoName, filename, callback);
  }
}
