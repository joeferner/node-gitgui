...
GitRepo.prototype.checkoutLocalFile = function(filename, callback) {
  filename = encodeURIComponent(filename);
  $.post(this.createUrl('/local/' + filename + '?action=checkout'),function(data, textStatus) {
    return callback(null, data);
  }).error(this.ajaxError.bind(this, 'checkoutLocalFile'));
};
